	...
	if (node)
	{
		...
		for (size_t i = 0; i < statDataMapSize; ++i)
		{
			if (const YAML::Node &dd = node[statDataMap[i].name])
			{
				if (dd.IsScalar())
				{
					_bonus.push_back(std::pair<BonusStatFunc,float>(statDataMap[i].func.power[0], dd.as<float>()));
				}
				else
				{
					for (size_t j = 0; j < statDataFuncSize && j < dd.size(); ++j)
					{
						...
						if (!AreSame(val, 0.0f))
						{
							_bonus.push_back(std::pair<BonusStatFunc,float>(statDataMap[i].func.power[j], val));
						}
					}
				}
			}
...
{
	_bonus.clear();
	_bonus.push_back(std::pair<BonusStatFunc,float>(&stat1<&UnitStats::firing>, 1.0f));
}
...
{
	_bonus.clear();
	_bonus.push_back(std::pair<BonusStatFunc,float>(&stat1<&UnitStats::melee>, 1.0f));
}
...
{
	_bonus.clear();
	_bonus.push_back(std::pair<BonusStatFunc,float>(&stat1<&UnitStats::throwing>, 1.0f));
}
...
{
	_bonus.clear();
	_bonus.push_back(std::pair<BonusStatFunc,float>(&stat2<&UnitStats::psiSkill, &UnitStats::psiStrength>, 0.02f));
}
...
{
	_bonus.clear();
	_bonus.push_back(std::pair<BonusStatFunc,float>(&stat1<&UnitStats::psiStrength>, 1.0f));
	_bonus.push_back(std::pair<BonusStatFunc,float>(&stat1<&UnitStats::psiSkill>, 0.2f));
}
...
{
	_bonus.clear();
	_bonus.push_back(std::pair<BonusStatFunc,float>(&stat0<100>, 1.0f));
}
...
{
	_bonus.clear();
	_bonus.push_back(std::pair<BonusStatFunc,float>(&stat1<&UnitStats::strength>, 1.0f));
}
...
{
	_bonus.clear();
	_bonus.push_back(std::pair<BonusStatFunc,float>(&stat1<&UnitStats::tu>, 1.0f));
}
...
{
	_bonus.clear();
	_bonus.push_back(std::pair<BonusStatFunc,float>(&basicEnergyRegeneration, 1.0f));
}
...
{
	_bonus.clear();
	_bonus.push_back(std::pair<BonusStatFunc,float>(&stat0<1>, 1.0f));
}
